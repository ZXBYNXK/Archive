...
const departmentsGlobal = {
  // <--- Created a GLOBAL OBJECT to keep everything in.
  information: {
    //<--- Decided to put information about the program in its own OBJECT incase anyone sees it.
  ...
  program: {
    //<--- Going to store ALL PROGRAM DATA in this OBJECT
    ...
    methods: {
      configureFiles: function () {
        let fs = require("fs"),
          ...
          employeeCareerStatFile_id3 = fs.readFileSync(
            "./load_dept_emp.txt",
            "utf8"
          ),
          departmentNamesFile_id4 = fs.readFileSync(
            "./load_dept_names.txt",
            "utf8"
          ),
          takeDataThenSort = () => {
            let salaries = salariesFile_id1
                .replace(/^\s*|\(|\'\)\,|INSERT INTO `salaries` VALUES /gm, "")
                .replace(/[(]/g, "")
                .replace(/\,\'|\'\,\'|\,|\n/gm, " ")
                .split(" "),
              employees = employeesFile_id2
                .replace(/^\s*|\'\)\,/gm, "")
                .replace(/[(]/g, "")
                .replace("INSERT INTO `employees` VALUES ", "")
                .replace(/\,\'|\'\,\'|\n/gm, " ")
                .split(" "),
              employeeCareerStats = employeeCareerStatFile_id3
                .replace(/^\s*|\(|\'\)\,/gm, "")
                .replace(/[(]/g, "")
                .replace("INSERT INTO `dept_emp` VALUES ", "")
                .replace(/\,\'|\'\,\'|\n/gm, " ")
                .split(" "),
              departmentNames = departmentNamesFile_id4
                .replace(/^\s*|\(\'|\'\)\;|\'\)\,/g, "")
                .replace(/[(']|INSERT INTO `departments` VALUES /g, "")
                .replace(/\,|\n/g, " ")
                .split(" "),
              ...
              covertArrayToObject_SendToSalaryArrayOfObjects = function () {
                ...
                salaries.forEach((currentIndexValueFromSalariesArray) => {
                  if (count === 1) {
                    ...
                    departmentsGlobal.program.salaryArrayOfObjects.push(
                      temporaryObject
                    );
              ...
              covertArrayToObject_SendToEmployeeArrayOfObjects = function () {
                ...
                employees.forEach((currentIndexValueFromSalariesArray) => {
                  if (count === 1) {
                    ...
                    count === 6 &&
                    !currentIndexValueFromSalariesArray.includes("s")
                  ) {
                    console.log(count, currentIndexValueFromSalariesArray);
                    console.log(`=====`);
