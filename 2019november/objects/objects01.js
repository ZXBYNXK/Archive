...
var runProgram = function () {
  ...
  var people = people.concat(
    new person("John F.", "Kennedy", `35th U.S. President`)
  );
  var people = people.concat(new person("Malcom", "X", `American Minster`));
  var people = people.concat(
    new person("D.R. J. Allen ", "Hynek", `American Astronomer`)
  );
  var people = people.concat(
    new person("Abraham", "Lincoln", `16th U.S. President`)
  );
  var people = people.concat(
    new person("Bob", "Marley", `Jamacian Singer-SongWriter`)
  );
  var people = people.concat(
    new person(
      "Hermes",
      "Trismegistus",
      `High Priest of Egyptian Mystery Schools`
    )
  );
  var people = people.concat(
    new person("John F.", "Nash Jr.", `American Mathematicion`)
  );
  var people = people.concat(
    new person("Albert", "Einstein", `Theoretical Physicist`)
  );
  ...
  var iteratePeopleArray = function () {
    ...
    for (let i = 0; i < people.length; i++) {
      console.log(
        `Person#:${i + 1} FIRSTNAME: ${people[i].firstName}, LASTNAME: ${
          people[i].lastName
