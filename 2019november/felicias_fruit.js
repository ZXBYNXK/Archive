...
let total = price * qty;
console.log(`Fruit: ${fruit} |Calculation: $${price} * ${qty} = $${total} |Revenue: $${total}`)
},
runProgram = function(){
if(availableFruitArr.length === fruitPricesArr.length && fruitPricesArr.length === fruitQtySoldArr.length && fruitQtySoldArr.length === availableFruitArr.length){
for(let i=0, j=0, k=0; i < availableFruitArr.length && j < fruitPricesArr.length && k < fruitQtySoldArr.length; i++, j++, k++){
