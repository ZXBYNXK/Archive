    ...
    const readline = require("readline"),
      ...
      felicasMethods = {
        sortFruitsPricesConigFile: function () {
          let fruitsAndPricesData = fs.readFileSync(
              "./.items_prices_config.txt",
              "utf8"
            ),
            change1 = fruitsAndPricesData
              .replace(
                /[\~|\!|\@|\#|$|\%|^|\&|*|\(|\)|_|\-|+|\,|?|\/|\\|]/gm,
                ""
              )
        ...
        displayInventoryCustomer: function () {
          ...
          felicaGlobal.felicasInventory.forEach((object) => {
            console.log(
              `Item#${felicaGlobal.felicasInventory.indexOf(object)}: ${
                object.fruit
              } ~ $${object.price}`
            );
            console.log("");
        ...
        addQtyToItem: function (itemName, qty) {
          felicaGlobal.felicasInventory.forEach((object) => {
            if (object.fruit === itemName) {
              object.qty += qty;
              console.log(
                `Sucessfully added ${qty} in qty to ${object.fruit} | Qty now: ${object.qty}`
              );
            }
          });
          felicasMethods.mainMenu();
        ...
        purchaseItemCheck: function (fruitName, qty) {
          ...
          felicaGlobal.felicasInventory.forEach((object) => {
            if (object.fruit === fruitName && object.qty >= qty) {
              ...
              if (
                felicaGlobal.felicasInventory.indexOf(object) ===
                felicaGlobal.felicasInventory.length - 1
              ) {
                if (confirmation === false) {
                  console.log(
                    `No such qty of fruit or fruit found:(${fruitName})`
                  );
                  felicasMethods.mainMenu();
                }
              }
        ...
        displayShortagesOnMenu: function () {
          felicaGlobal.felicasInventory.forEach((object) => {
            if (object.runningLow === true) {
              console.log(
                `Running short on ${object.fruit}, Qty:${object.qty}`
              );
            }
          });
        },
        askDefaultValue: function () {
          ...
          console.log(
            `If you want each fruit's qty to be different a different number then use the program normally, by selecting option "3333". `
          );
          console.log(
            `This is only useful to test the program and or to add qauntites at faster rate as long as all the number of items are equal.`
          );
        ...
        trashedFruits: function () {
          ...
          rl.question(``, function (desiredFruit) {
            ...
            rl.question(``, function (desiredQty) {
              console.log(
                `You are about to trash ${desiredQty} ${desiredFruit}.`
              );
              console.log("Continue [Y/n]?");
          ...
          felicaGlobal.felicasInventory.forEach((object) => {
            if (object.fruit === fruit) {
              if (object.qty >= qty) {
                ...
                console.log(
                  `Fruit:${object.fruit} Now has quantity ${object.qty}`
                );
                felicasMethods.mainMenu();
              } else {
                console.log(
                  `Qty to trash:${qty} is exceeds ${
                    (object.fruit, object.qty)
                  } `
                );
                felicasMethods.mainMenu();
              }
        ...
        mainMenu: function () {
          ...
          console.log(
            `1111: Show inventory information. | 2222: Dislpay shortages. | 3333: Add qty to item.`
          );
          console.log("");
          console.log(
            `4444: Set all quanties to one number, recomended to be faster if all items being added share an equality, and for program testing.`
          );
          console.log("");
