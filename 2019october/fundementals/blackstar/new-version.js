...
var adminPage = function () {
  console.log(logo);
  console.log(
    `What do you want to do Mr.Admin?     See all account information?('all info')  See all agents registered('all agents') `
  );
  console.log(
    `                                      Create an agent account('create agent')   Create new Admin('create admin')`
  );
          ...
          if (backupAccounts[i].includes(name)) {
            if (backupAccounts[i].includes(id)) {
              if (backupAccounts[i].includes(password)) {
                ...
                console.log(``);
                console.log(
                  `Succesfully logged in user: ${name}    No Such Agency Black Star Communications Network    Client:${id}     Date:${date}`
                );
                console.log(logo);

                console.log(
                  `Head Back to landing Screen('home')?  Check Messages('messages') See Registered('agents')`
                );
                rl.question(``, function (choice) {
                  switch (choice) {
                    case "home":
                ...
                var incorrectEntry = function () {
                  console.log(
                    `Incorrect NAME:(${name}), ID:(${id}) or PASSWORD:(${password})`
                  );
            ...
            rl.question(``, function (id) {
              ...
              rl.question(``, function (id2) {
                if (id == id2) {
                  ...
                  let tempArr2 = [
                    `([MILITARY~NAME/ID#/Password]:~${name} ${id} ${password})`,
                  ];
                  agentAccounts = agentAccounts.concat(tempArr);
                  ...
                  console.log(
                    `There has been an error in confirmation please try again.`
                  );
...
var theEval = function () {
  ...
  console.log(`\n\n`);
  console.log(
    `(TO REGISTER A NEW ACCOUNT FOR A NEW AGENT..... Type : 'new'),  (To Continue To Blackstar Login....     Type: 'log)`
  );
