...
function deptemp() {
  ...
  departmentEmp = departmentEmp
    .replace(/INSERT INTO `dept_emp` VALUES |\(|\)|\'/g, "")
    .replace(/^\s*/gm, "")
    .split("\n");

  for (let i = 0; i < departmentEmp.length; i++) {
    departmentEmp[i] = departmentEmp[i].substring(
      0,
      departmentEmp[i].length - 1
    );
...
function totalEmp() {
  ...
  for (let j = 0; j < departmentNames.length; j++) {
    for (let k = 0; k < departmentEmp.length; k++) {
      if (
        departmentEmp[k][1] == departmentNames[j][0] &&
        departmentEmp[k].includes("9999")
      ) {
